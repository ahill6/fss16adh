...
XX, yy = readInput('./DataMining/millionSongString.txt')
...
for train_index, test_index in kf.split(XX):
    print("start")
    i += 1
    training, testing = [XX[j] for j in train_index], [XX[j] for j in test_index]
    classes, actuals = [yy[j] for j in train_index], [yy[j] for j in test_index]
    tr = spatialtree(training)
    results = [{'tp':0, 'tn':0, 'fp':0, 'fn':0} for _ in range(splits)]
    for t in range(len(testing)):
          if t%1000 == 0:
                print(t)

          l = [yy[j] for j in tr.k_nearest(training, k=kNearest, vector=testing[t])]
          pred = (sum(l)+0.0)/len(l)
          if actuals[t] == 1:
                if pred > 0.5:
                      results[i]['tp'] += 1
                else:
