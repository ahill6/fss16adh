...
class Factory(O):
    ...
    def make_machine(self, label, data=0):
        ...
        a = Machine(label=label, data=data)
...
def fsm(factory, label, data):
    ...
    # TODO 8: Using the functions in classes above, code up the
    # state machine in the figure on top.
    start = m.add_state("Start")
    outside = m.add_state("#Outside")
    dead = m.add_state("Dead.")
    m.add_trans(Trans(start, walk, outside), Trans(outside, snow, outside), Trans(outside, grass, outside), Trans(outside, pit, dead))
    return m
