...
class Solution(O):
    ...
    def new_features(self, p, problem):
        successes = int(min(round((p/25.0)/random.random()), len(self.features)))
    ...
    def competitor_features(self, p):
        ...
        successes = int(min(round((p/250000.0)/random.random()), len(self.opponent_features - self.features)))
...
class Problem(O):
    ...
    def __init__(self, filename, companies=0):
        ...
        O.__init__(self, decisions=decs, objectives=objs, constraints=consts, omax=omax, my_features=mine, opponent_features=theirs, all_features_data=initfeats, all_features = allfeattmp)
    ...
    def generate_one(self, opponent=None):
        ...
        retries = 2500 #make this configurable
        while retries > 0:
            if opponent == None:
