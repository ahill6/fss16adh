...
class pom3_requirements:
    def __init__(requirements, num_decisions, interdependency):
        requirements.heap = requirements_tree()
        requirements.count = int(2.5*[3,10,30,100,300][num_decisions])
        requirements.interdependency = interdependency

        for i in range(requirements.count):
            requirements.heap.add_root(Requirement((2)*random_cost(), random_value()), 'Base Req #' + str('%.3d'%(i+1)))
        ...
        for i in range(requirements.count):
            ...
            if (rand <= 15):
                ...
                if ((i+1) < len(requirements.heap.tree)):
                    ...
                    if adderDie <= interdependency: requirements.add_dependency(requirements.heap.tree[i], req_node)
            requirements.recursive_dep_adder(requirements.heap.tree[i], i, 1)


        #linearize the list
        requirements.tasks = requirements.heap.traverse()
        #print(requirements.tasks)
    ...
    def add_children(self, num, parent, level):
        for c in range(num):
            ...
            tmp = random_value()
            count = 0
            while tmp < x and count < 100:
                tmp = random_value()
                count += 1
            y = tmp 
            parent.add_child(Requirement(x, y), "+"*level + 'Child-' + parent.key[0] + parent.key[len(parent.key)-3] + parent.key[len(parent.key)-2] + parent.key[len(parent.key)-1] + ' #' + str('%.3d'%(c+1)), level)
        ...
        if (len(parent.children) > 0 and ((rootIndex+1) < len(self.heap.tree))):
            if (level <= self.heap.tree[rootIndex+1].max_depth()):
                ...
                if (rand <= odds[oddsInd]):
                    ...
                    adderDie = random.randint(1,100)
                    if adderDie <= self.interdependency: self.add_dependency(randChild, levelNodes[rand])
